...
tags$style(HTML("b, strong{color:#CD1076;
		...
		")),
                        ...
                        selectInput('storytelling', 'GeoStory to Share', ""),
...
if (is.data.frame(a1$a)==TRUE) {
       ...
       print(paste(a1$my_path,"/index.html",sep="")),
#             a('Click to launch the info map',target='_blank',href='index.html'),
             "</b><br><br> In addition you can locate the <b>'index.html'</b> and the folder called <b>'lib'</b> in the following directory: <br><br><b>", print(paste(a1$my_path, sep="")),
...
# a$woman <- gsub("_", "/", as.character(a$woman))
a <- a[!is.na(a$shape),]
print(str(a))

 if ("woman" %in% colnames(a)==TRUE) {
a$videophudcfily <- substr(a$woman, 1, 5)#ifelse("woman" %in% colnames(a)==TRUE, substr(a$woman, 1, 5), NULL)
a$woman <- ifelse ((a$videophudcfily == "<ifra" | a$videophudcfily == "https"), paste0(a$woman), paste("file://", a$woman, sep=""))
 } else {
...
addControl(mtitle, position = "topright", className="map-title")%>%
  ...
  leaflet::addLegend(position = "bottomright",
            ...
            values = a$ego, # data frame column for legend
...
my_path <- tempdir()
a1$my_path <- my_path
saveWidget(widget = map, file = paste(my_path, "/index.html", sep=""), selfcontained = FALSE, libdir = "lib")
utils::browseURL(paste(my_path, "/index.html", sep=""))
          tags$iframe( #Everything before iframe or it crashes
              src = paste(my_path, "/index.html", sep=""),
              width = "0%",
              height = "0px",
